...
formButton.addEventListener("click", (e)=>{
    ...
    if (positionInput.value.length < 4) {
        showNotification("Error", "Position must be at least 4 letters long.", "error");
        return;
    }
    [
        ...formInputs
        ...
        if (input.type !== "button") {
            ...
            } else cell.innerText = input.value;
            row.append(cell);
        }
    });
    ...
    [
        ...
        cell.addEventListener("dblclick", ()=>{
            if (currentEditCell && currentEditCell !== cell) {
                ...
                if (previousInput) previousInput.replaceWith(currentEditCell);
            ...
            input.addEventListener("keydown", (e)=>{
                if (e.key === "Enter") handleCellEditCompletion(input, cell, originalValue);
            });
...
[
    ...
    cell.addEventListener("click", (e)=>{
        ...
        const sortedRows = [
            ...
            if (e.target === cellAge || e.target === cellSalary) return isAsc ? +item2.replace(/[$,]/g, "") - +item1.replace(/[$,]/g, "") : +item1.replace(/[$,]/g, "") - +item2.replace(/[$,]/g, "");
            else return isAsc ? item2.localeCompare(item1) : item1.localeCompare(item2);
        });
        sortedRows.forEach((row)=>table.tBodies[0].append(row));
        Object.keys(sortOrder).forEach((key)=>{
            if (+key !== +e.target.cellIndex) delete sortOrder[key];
        });
    });
});
...
function handleCellEditCompletion(input, cell, originalValue) {
    if (input.value.trim().length >= 4) cell.innerText = input.value;
    else {
        showNotification("Error", "Name must be at least 4 letters long.", "error");
